...
function NavLink({
  ...
  return <a href={href} className="flex items-center gap-1 px-2.5 py-2 rounded-full text-[0.62rem] font-medium tracking-wide uppercase hover:bg-white/8 transition-all duration-200 whitespace-nowrap" style={{
    fontFamily: 'Satoshi, system-ui, sans-serif',
    color: '#D9D9D9',
    mixBlendMode: 'difference'
  }}>
      {label}
      {chevron && <ChevronDown size={8} strokeWidth={2.5} style={{
      color: '#D9D9D9',
      mixBlendMode: 'difference',
      marginTop: '1px'
    }} />}
...
function HireBtn() {
  return <button onClick={() => window.dispatchEvent(new CustomEvent('open-contact-modal'))} className="group flex items-center bg-white rounded-full overflow-hidden hover:bg-white/80 transition-colors duration-200 shrink-0">
      <span className="pl-4 pr-1.5 py-1.75 text-black text-[0.61rem] font-medium tracking-[0.14em] uppercase whitespace-nowrap" style={{
      fontFamily: 'Satoshi, system-ui, sans-serif'
    }}>
        Hire Me
      </span>
...
export function Navbar() {
  ...
  return <>
      ...
      <div data-theme="dark" className="fixed top-0 left-0 right-0 z-50 hidden md:block pointer-events-none">
        ...
        <motion.div animate={{
          width: targetW
        }} transition={SPRING} className="pointer-events-auto flex items-center rounded-full" style={{
          ...
          boxShadow: scrolled ? '0 4px 24px rgba(0,0,0,0.35), inset 0 1px 0 rgba(255,255,255,0.06)' : '0 8px 40px rgba(0,0,0,0.50), inset 0 1px 0 rgba(255,255,255,0.08)',
      ...
      <div className="fixed top-0 left-0 right-0 z-50 md:hidden pointer-events-none">
        ...
        <span style={{ fontFamily: 'Satoshi, system-ui, sans-serif', fontWeight: 800, letterSpacing: '-0.02em', color: '#fff' }}>
          ...
          boxShadow: '0 6px 28px rgba(0,0,0,0.40)'
            ...
            <div className="flex flex-col gap-1.25 w-3.75">
              <motion.span animate={mobileOpen ? {
              rotate: 45,
              y: 5.5
            } : {
              rotate: 0,
        ...
        <AnimatePresence>
          ...
          ease: [0.22, 1, 0.36, 1]
              ...
              <button onClick={() => setMobileOpen(false)} className="absolute top-5 right-5 w-10 h-10 rounded-full bg-white/8 flex items-center justify-center hover:bg-white/15 transition-colors" aria-label="Close menu">
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                  <path d="M1 1l12 12M13 1L1 13" stroke="#FFFFFF" strokeWidth="1.5" strokeLinecap="round" />
                </svg>
              </button>

              {NAV_LINKS.map((link, i) => <motion.a key={link.label} href={link.href} onClick={() => setMobileOpen(false)} initial={{
            opacity: 0,
            y: 16
          }} animate={{
            opacity: 1,
            ...
            ease: [0.22, 1, 0.36, 1]
                  ...
                  <span className="font-black tracking-[-0.04em] text-white" style={{
              fontFamily: 'Satoshi, system-ui, sans-serif',
              fontWeight: 800,
            ...
            duration: 0.3
              ...
              window.dispatchEvent(new CustomEvent('open-contact-modal'));
                  ...
                  <span className="text-[0.68rem] font-medium tracking-[0.18em] uppercase" style={{
                fontFamily: 'Satoshi, system-ui, sans-serif'
              }}>
                    Hire Me
                  </span>
